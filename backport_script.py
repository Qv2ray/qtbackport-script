...
def main():
    ...
    if main_sel == 0:
        ...
        if mode_sel == 1:
            ...
            find . -name control -exec sed -i '/qdoc-qt5 (>= 5.14~) <!nodoc>/d' {} +
            find . -name control -exec sed -i '/qhelpgenerator-qt5 (>= 5.14~) <!nodoc>/d' {} +
            find . -name control -exec sed -i '/qtattributionsscanner-qt5 (>= 5.14~) <!nodoc>/d' {} +
            find . -name control -exec sed -i '/qtbase5-doc-html (>= 5.14+dfsg~) <!nodoc>/d' {} +
            find . -name control -exec sed -i '/qttools5-dev-tools (>= 5.14~) <!nodoc>/d' {} +
            find . -name control -exec sed -i '/qtwebsockets5-doc-html (>= 5.14~) <!nodoc>/d' {} +
            """
            subprocess.call(command, shell=True)
            for i in file_path:
